...
> **Multi-tenant Customer Advisory Board (CAB) platform — "Advisory-as-a-Service".**
...
**Kubernetes namespace**: `coforma-studio`
...
### Day-to-day for coforma-studio-api

The commands below default to `coforma-studio-api` — the primary service name for
...
enclii ps --wide
enclii ps coforma-studio-api --env production

# Logs (tail, filter, history)
enclii logs coforma-studio-api -f                          # live tail
enclii logs coforma-studio-api --since 1h --level error    # last hour, errors only
enclii logs coforma-studio-api --env staging -f
...
# Rollback
enclii rollback coforma-studio-api                         # previous release
enclii rollback coforma-studio-api --to-revision 5

# Releases + history
enclii releases coforma-studio-api                          # list builds
enclii releases coforma-studio-api --latest --output json

# Secrets (routed through Lockbox → Vault → ESO → K8s)
enclii secrets list coforma-studio-api
enclii secrets set MY_KEY=value --service coforma-studio-api --secret
enclii secrets rm MY_KEY --service coforma-studio-api

# Domains, tunnel routes, DNS
enclii domains list coforma-studio-api
enclii domains add coforma-studio-api my.example.com       # auto-provisions tunnel route + DNS
...
# Routing (ingress + TLS)
enclii junctions list coforma-studio-api
